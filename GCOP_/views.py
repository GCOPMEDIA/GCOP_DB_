...
def update_session_data(request, new_data):
    ...
    user_data = json.loads(
        request.session.get("final_data5", "{}")
    )  # Load existing session data
    user_data.update(new_data)  # Merge new data
    request.session["final_data5"] = json.dumps(
        user_data, cls=DjangoJSONEncoder
    )  # Store back
...
def further_questions_view(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            request.session["number_of_children"] = int(
                cleaned_data.get("number_of_children", 0)
            )
            request.session["number_of_survivors"] = int(
                cleaned_data.get("number_of_survivors", 0)
            )
            request.session["marital_status"] = cleaned_data.get(
...
def child_details_view(request, child_index):
    ...
    return render(
        request,
        "form_template.html",
        {"form": form, "step": "3", "child_index": child_index},
    )
...
def survivor_details_view(request, survivor_index):
    ...
    return render(
        request,
        "form_template.html",
        {"form": form, "step": "7", "survivor_index": survivor_index},
    )
...
def form_success_view(request):
    ...
    try:
        ...
        error = "Member with this number is already registered"
        messages.error(request, error)  # Add this line
...
def member_form_view(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect(
                "users_search_view",
                f_name=f_name or "none",
                l_name=l_name or "none",
    ...
    try:
        ...
        with open(output_path, "rb") as pdf_file:
            ...
            response["Content-Disposition"] = (
                f'attachment; filename="member_{member_id}.pdf"'
            )
            return response
...
def card_details(request, member_id):
    ...
    return render(
        ...
        {
            "member": member,
            "church_branch": church_branch.branch_name,
            "qr_code": qr_base64,
            "church_id": church_id(member_id),
            "image": member_image,
            "name": formatted_name,
            "f_id": formatted_id,
        },
...
def attendance(request):
    ...
    members = Attendance.objects.filter(scanned_at__date=today).select_related("member")
...
def attendance_mercy_temple(request):
    ...
    # Fetch all attendance records for today and related members
    members = AttendanceMercyTemple.objects.filter(
