...
class UserDetailsForm(forms.Form):
    ...
    emergency = forms.CharField(
        max_length=15, label="Emergency Contact", required=False
    )
    occupation = forms.CharField(max_length=100, label="Occupation", required=False)
    nxt_of_kin = forms.CharField(
        max_length=100, label="Name Of Next Of Kin", required=False
    )
    address = forms.CharField(max_length=100, label="Ghana Post Address")
    place_of_residence = forms.CharField(
        max_length=200, label="Place Of Residence", required=False
    )
    hometown = forms.CharField(max_length=100, label="Home Town")
    gender = forms.ChoiceField(
        choices=gender_choices, widget=forms.Select, label="Gender"
    )
    baptism = forms.ChoiceField(
        choices=yes_or_no, widget=forms.Select, label="Have You Been Baptized? "
    )
    baptist_at_gcop = forms.ChoiceField(
        choices=yes_or_no, widget=forms.Select, label="Were You Baptized At GCOP?"
    )
    history = forms.CharField(
        widget=forms.Textarea(attrs={"row": 4, "cols": 100}), label="Brief History"
    )


class FurtherQuestionsForm(forms.Form):
    marital_status = forms.ChoiceField(
        choices=married_status, widget=forms.Select, label="Marital Status"
    )
    ...
    parent_status = forms.ChoiceField(
        label="Are Your Parents Still Alive?", choices=parents
    )
    date_joined = forms.DateField(
        label="What Year Did You Join GCOP", required=False, help_text="mm/dd/yy"
    )
    welfare_card_number = forms.CharField(
        max_length=200, label="Welfare Card Number if any", required=False
    )
    tithe_card_number = forms.CharField(
        max_length=200, label="Tithe And Dues Card Number if any", required=False
    )
    church_branch = forms.ChoiceField(choices=church_branches, widget=forms.Select)
    group_name = forms.MultipleChoiceField(
        choices=group_choices,
        widget=forms.SelectMultiple(attrs={"class": "multi-dropdown"}),
    )
...
class NextForm(forms.Form):
    ...
    child_phone_number = forms.CharField(
        max_length=15, label="Child's Phone Number", required=False
    )
...
class FatherForm(forms.Form):
    ...
    father_phone_number = forms.CharField(
        max_length=15, label="Father's Phone Number", required=False
    )
    father_is_member = forms.ChoiceField(
        choices=yes_or_no, widget=forms.Select, label="Is Your Father A Member Of GCOP?"
    )
...
class MotherForm(forms.Form):
    ...
    mother_phone_number = forms.CharField(
        max_length=15, label="Mother's Phone Number", required=False
    )
    mother_is_member = forms.ChoiceField(
        choices=yes_or_no, widget=forms.Select, label="Is Your Mother A Member Of GCOP?"
    )


class SurvivorForm(forms.Form):
    survivor_first_name = forms.CharField(max_length=250, label="Relative's Surname")
    survivor_other_name = forms.CharField(
        max_length=250, label="Relative's Other Names"
    )
    survivor_phone_number = forms.CharField(
        max_length=15, label="Relative's Phone Number", required=False
    )
    survivor_relation = forms.ChoiceField(
        choices=relation, widget=forms.Select, label="Relationship With Relative"
    )
    survivor_is_member = forms.ChoiceField(
        choices=yes_or_no,
        widget=forms.Select,
...
class SpouseForm(forms.Form):
    ...
    spouse_is_member = forms.ChoiceField(
        choices=yes_or_no, widget=forms.Select, label="Is Your Spouse A Member Of GCOP?"
    )
...
class EditMemberForm(forms.ModelForm):
    """ModelForm for editing Member instances in the admin UI.

    Uses the Member model fields we commonly update in the app.
    """
...
#         'nxt_of_kin': 'Next of Kin',
#         'address': 'Address',
#         'place_of_residence': 'Place of Residence',
#         'hometown': 'Hometown',
#         'gender': 'Gender',
#         'baptism': 'Baptism',
#         'baptist_at_gcop': 'Baptized at GCOP',
#         'history': 'History',
#     }
